...
//       await Status.create({userId, mediaUrl: media.url, caption});
//     });
//     // );
...
//   try {
//     const {statusId, userId, text} = req.body;
...
// // Get Status Seen List
// export const getStatusSeenList = async (req, res) => {
//   try {
...
//       'name profilePic'
//     );
//     res.status(200).json({success: true, viewers: status.viewers});
//   } catch (error) {
//     res.status(500).json({success: false, message: error.message});
//   }
// };
...
//   try {
//     const userId = req.user._id;
//     const status = await User.findById(userId).populate(
//       'viewers',
//       'name profilePic'
//     );
//     res.status(200).json({success: true, viewers: status.viewers});
